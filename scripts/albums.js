...
// ---------------------------------------------------------------------------
// Legacy music page (hero + discography grid with modal)
// ---------------------------------------------------------------------------
...
export async function initMusicPage() {
  ...
  try {
    ...
    const albums = raw.map(normalizeAlbum).sort(sortByDateDesc);
    renderHero(heroEl, albums);
    renderDiscography(gridEl, albums);
  } catch (err) {
    console.error('[albums] initMusicPage failed', err);
...
function normalizeAlbum(a) {
  ...
  return {
    ...
    cover: a.cover || `${CONFIG.coversDir}/${slug}.webp`,
    featured: !!a.featured,
    slug,
  };
}
